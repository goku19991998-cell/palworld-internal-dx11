...
void* allocate_near(uintptr_t target, size_t size)
{
	SYSTEM_INFO sysInfo;
	GetSystemInfo(&sysInfo);
	const uintptr_t allocGranularity = sysInfo.dwAllocationGranularity;

	const uintptr_t rangeMin = (target > 0x7FFF0000) ? (target - 0x7FFF0000) : 0x10000;
	const uintptr_t rangeMax = target + 0x7FFF0000;

	uintptr_t startAddr = target - (target % allocGranularity);

	for (uintptr_t addr = startAddr; addr > rangeMin && addr < rangeMax; )
	{
		void* mem = VirtualAlloc(reinterpret_cast<void*>(addr), size, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
		if (mem)
			return mem;
