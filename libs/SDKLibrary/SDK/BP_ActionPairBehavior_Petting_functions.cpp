#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: BP_ActionPairBehavior_Petting

#include "Basic.hpp"

#include "BP_ActionPairBehavior_Petting_classes.hpp"
#include "BP_ActionPairBehavior_Petting_parameters.hpp"


SDK_NAMESPACE_START

// Function BP_ActionPairBehavior_Petting.BP_ActionPairBehavior_Petting_C.GetEActionForRideBoss
// (Public, HasOutParams, BlueprintCallable, BlueprintEvent, BlueprintPure)
// Parameters:
// EPalActionType                          ReturnValue                                            (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

EPalActionType UBP_ActionPairBehavior_Petting_C::GetEActionForRideBoss()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_ActionPairBehavior_Petting_C", "GetEActionForRideBoss");

	Params::BP_ActionPairBehavior_Petting_C_GetEActionForRideBoss Parms{};

	UObject::ProcessEvent(Func, &Parms);

	return Parms.ReturnValue;
}


// Function BP_ActionPairBehavior_Petting.BP_ActionPairBehavior_Petting_C.GetHumanAnime
// (Protected, HasOutParams, BlueprintCallable, BlueprintEvent, BlueprintPure)
// Parameters:
// class UAnimMontage**                    Montage                                                (Parm, OutParm, ZeroConstructor, NoDestructor, HasGetValueTypeHash)

void UBP_ActionPairBehavior_Petting_C::GetHumanAnime(class UAnimMontage** Montage)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_ActionPairBehavior_Petting_C", "GetHumanAnime");

	Params::BP_ActionPairBehavior_Petting_C_GetHumanAnime Parms{};

	UObject::ProcessEvent(Func, &Parms);

	if (Montage != nullptr)
		*Montage = Parms.Montage;
}


// Function BP_ActionPairBehavior_Petting.BP_ActionPairBehavior_Petting_C.OnCompleted
// (Protected, BlueprintCallable, BlueprintEvent)
// Parameters:
// class AActor*                           Human                                                  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, NoDestructor, HasGetValueTypeHash)
// class AActor*                           Monster                                                (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, NoDestructor, HasGetValueTypeHash)

void UBP_ActionPairBehavior_Petting_C::OnCompleted(class AActor* Human, class AActor* Monster)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_ActionPairBehavior_Petting_C", "OnCompleted");

	Params::BP_ActionPairBehavior_Petting_C_OnCompleted Parms{};

	Parms.Human = Human;
	Parms.Monster = Monster;

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_ActionPairBehavior_Petting.BP_ActionPairBehavior_Petting_C.SpawnNiagara
// (Public, HasOutParams, BlueprintCallable, BlueprintEvent)
// Parameters:
// const struct FVector&                   Location                                               (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// class UObject*                          Object                                                 (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, NoDestructor, HasGetValueTypeHash)
// class UNiagaraComponent*                ReturnValue                                            (Parm, OutParm, ZeroConstructor, ReturnParm, InstancedReference, NoDestructor, HasGetValueTypeHash)

class UNiagaraComponent* UBP_ActionPairBehavior_Petting_C::SpawnNiagara(const struct FVector& Location, class UObject* Object)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_ActionPairBehavior_Petting_C", "SpawnNiagara");

	Params::BP_ActionPairBehavior_Petting_C_SpawnNiagara Parms{};

	Parms.Location = std::move(Location);
	Parms.Object = Object;

	UObject::ProcessEvent(Func, &Parms);

	return Parms.ReturnValue;
}


SDK_NAMESPACE_END
