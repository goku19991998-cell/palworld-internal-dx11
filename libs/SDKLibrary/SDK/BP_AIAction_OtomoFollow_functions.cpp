#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: BP_AIAction_OtomoFollow

#include "Basic.hpp"

#include "BP_AIAction_OtomoFollow_classes.hpp"
#include "BP_AIAction_OtomoFollow_parameters.hpp"


SDK_NAMESPACE_START

// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.ActionAbort
// (Event, Public, BlueprintEvent)
// Parameters:
// class APawn*                            ControlledPawn                                         (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, NoDestructor, HasGetValueTypeHash)

void UBP_AIAction_OtomoFollow_C::ActionAbort(class APawn* ControlledPawn)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "ActionAbort");

	Params::BP_AIAction_OtomoFollow_C_ActionAbort Parms{};

	Parms.ControlledPawn = ControlledPawn;

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.ActionFinished
// (Event, Public, BlueprintEvent)
// Parameters:
// class APawn*                            ControlledPawn                                         (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, NoDestructor, HasGetValueTypeHash)
// EPawnActionResult                       WithResult                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UBP_AIAction_OtomoFollow_C::ActionFinished(class APawn* ControlledPawn, EPawnActionResult WithResult)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "ActionFinished");

	Params::BP_AIAction_OtomoFollow_C_ActionFinished Parms{};

	Parms.ControlledPawn = ControlledPawn;
	Parms.WithResult = WithResult;

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.ActionPause
// (Event, Public, BlueprintEvent)
// Parameters:
// class APawn*                            ControlledPawn                                         (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, NoDestructor, HasGetValueTypeHash)

void UBP_AIAction_OtomoFollow_C::ActionPause(class APawn* ControlledPawn)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "ActionPause");

	Params::BP_AIAction_OtomoFollow_C_ActionPause Parms{};

	Parms.ControlledPawn = ControlledPawn;

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.ActionResume
// (Event, Public, BlueprintEvent)
// Parameters:
// class APawn*                            ControlledPawn                                         (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, NoDestructor, HasGetValueTypeHash)

void UBP_AIAction_OtomoFollow_C::ActionResume(class APawn* ControlledPawn)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "ActionResume");

	Params::BP_AIAction_OtomoFollow_C_ActionResume Parms{};

	Parms.ControlledPawn = ControlledPawn;

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.ActionStart
// (Event, Public, BlueprintEvent)
// Parameters:
// class APawn*                            ControlledPawn                                         (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, NoDestructor, HasGetValueTypeHash)

void UBP_AIAction_OtomoFollow_C::ActionStart(class APawn* ControlledPawn)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "ActionStart");

	Params::BP_AIAction_OtomoFollow_C_ActionStart Parms{};

	Parms.ControlledPawn = ControlledPawn;

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.ActionTick
// (Event, Public, BlueprintEvent)
// Parameters:
// class APawn*                            ControlledPawn                                         (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, NoDestructor, HasGetValueTypeHash)
// float                                   DeltaSeconds                                           (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UBP_AIAction_OtomoFollow_C::ActionTick(class APawn* ControlledPawn, float DeltaSeconds)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "ActionTick");

	Params::BP_AIAction_OtomoFollow_C_ActionTick Parms{};

	Parms.ControlledPawn = ControlledPawn;
	Parms.DeltaSeconds = DeltaSeconds;

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.ChangeStateByRange
// (Public, BlueprintCallable, BlueprintEvent)

void UBP_AIAction_OtomoFollow_C::ChangeStateByRange()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "ChangeStateByRange");

	UObject::ProcessEvent(Func, nullptr);
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.DelayTrainerYaw
// (Public, BlueprintCallable, BlueprintEvent)

void UBP_AIAction_OtomoFollow_C::DelayTrainerYaw()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "DelayTrainerYaw");

	UObject::ProcessEvent(Func, nullptr);
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.DoWarpNearPlayer
// (Private, HasDefaults, BlueprintCallable, BlueprintEvent)

void UBP_AIAction_OtomoFollow_C::DoWarpNearPlayer()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "DoWarpNearPlayer");

	UObject::ProcessEvent(Func, nullptr);
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.ExecuteUbergraph_BP_AIAction_OtomoFollow
// (Final, UbergraphFunction)
// Parameters:
// int32                                   EntryPoint                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UBP_AIAction_OtomoFollow_C::ExecuteUbergraph_BP_AIAction_OtomoFollow(int32 EntryPoint)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "ExecuteUbergraph_BP_AIAction_OtomoFollow");

	Params::BP_AIAction_OtomoFollow_C_ExecuteUbergraph_BP_AIAction_OtomoFollow Parms{};

	Parms.EntryPoint = EntryPoint;

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.ForwardRayCheckJump
// (Public, HasOutParams, HasDefaults, BlueprintCallable, BlueprintEvent)
// Parameters:
// bool*                                   DoJump                                                 (Parm, OutParm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UBP_AIAction_OtomoFollow_C::ForwardRayCheckJump(bool* DoJump)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "ForwardRayCheckJump");

	Params::BP_AIAction_OtomoFollow_C_ForwardRayCheckJump Parms{};

	UObject::ProcessEvent(Func, &Parms);

	if (DoJump != nullptr)
		*DoJump = Parms.DoJump;
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.GetTargetLocationInAim
// (Public, HasOutParams, HasDefaults, BlueprintCallable, BlueprintEvent)
// Parameters:
// struct FVector*                         Location                                               (Parm, OutParm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UBP_AIAction_OtomoFollow_C::GetTargetLocationInAim(struct FVector* Location)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "GetTargetLocationInAim");

	Params::BP_AIAction_OtomoFollow_C_GetTargetLocationInAim Parms{};

	UObject::ProcessEvent(Func, &Parms);

	if (Location != nullptr)
		*Location = std::move(Parms.Location);
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.GetTargetLocationInBuildingMode
// (Public, HasOutParams, HasDefaults, BlueprintCallable, BlueprintEvent)
// Parameters:
// struct FVector*                         Location                                               (Parm, OutParm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UBP_AIAction_OtomoFollow_C::GetTargetLocationInBuildingMode(struct FVector* Location)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "GetTargetLocationInBuildingMode");

	Params::BP_AIAction_OtomoFollow_C_GetTargetLocationInBuildingMode Parms{};

	UObject::ProcessEvent(Func, &Parms);

	if (Location != nullptr)
		*Location = std::move(Parms.Location);
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.GetTargetLocationInFishing
// (Public, HasOutParams, HasDefaults, BlueprintCallable, BlueprintEvent)
// Parameters:
// struct FVector*                         Location                                               (Parm, OutParm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UBP_AIAction_OtomoFollow_C::GetTargetLocationInFishing(struct FVector* Location)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "GetTargetLocationInFishing");

	Params::BP_AIAction_OtomoFollow_C_GetTargetLocationInFishing Parms{};

	UObject::ProcessEvent(Func, &Parms);

	if (Location != nullptr)
		*Location = std::move(Parms.Location);
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.GetTargetLocationInHighSpeed
// (Public, HasOutParams, HasDefaults, BlueprintCallable, BlueprintEvent)
// Parameters:
// struct FVector*                         Location                                               (Parm, OutParm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UBP_AIAction_OtomoFollow_C::GetTargetLocationInHighSpeed(struct FVector* Location)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "GetTargetLocationInHighSpeed");

	Params::BP_AIAction_OtomoFollow_C_GetTargetLocationInHighSpeed Parms{};

	UObject::ProcessEvent(Func, &Parms);

	if (Location != nullptr)
		*Location = std::move(Parms.Location);
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.GetTargetLocationInWalk
// (Public, HasOutParams, BlueprintCallable, BlueprintEvent)
// Parameters:
// struct FVector*                         Location                                               (Parm, OutParm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UBP_AIAction_OtomoFollow_C::GetTargetLocationInWalk(struct FVector* Location)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "GetTargetLocationInWalk");

	Params::BP_AIAction_OtomoFollow_C_GetTargetLocationInWalk Parms{};

	UObject::ProcessEvent(Func, &Parms);

	if (Location != nullptr)
		*Location = std::move(Parms.Location);
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.IsBuildingMode
// (Public, HasOutParams, BlueprintCallable, BlueprintEvent, BlueprintPure)
// Parameters:
// bool*                                   IsBuilding                                             (Parm, OutParm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UBP_AIAction_OtomoFollow_C::IsBuildingMode(bool* IsBuilding)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "IsBuildingMode");

	Params::BP_AIAction_OtomoFollow_C_IsBuildingMode Parms{};

	UObject::ProcessEvent(Func, &Parms);

	if (IsBuilding != nullptr)
		*IsBuilding = Parms.IsBuilding;
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.IsFishing
// (Public, HasOutParams, BlueprintCallable, BlueprintEvent, BlueprintPure)
// Parameters:
// bool*                                   IsFishing_0                                            (Parm, OutParm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UBP_AIAction_OtomoFollow_C::IsFishing(bool* IsFishing_0)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "IsFishing");

	Params::BP_AIAction_OtomoFollow_C_IsFishing Parms{};

	UObject::ProcessEvent(Func, &Parms);

	if (IsFishing_0 != nullptr)
		*IsFishing_0 = Parms.IsFishing_0;
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.IsTurnInBuildingMode
// (Public, HasOutParams, BlueprintCallable, BlueprintEvent, BlueprintPure)
// Parameters:
// bool*                                   IsTurn                                                 (Parm, OutParm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UBP_AIAction_OtomoFollow_C::IsTurnInBuildingMode(bool* IsTurn)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "IsTurnInBuildingMode");

	Params::BP_AIAction_OtomoFollow_C_IsTurnInBuildingMode Parms{};

	UObject::ProcessEvent(Func, &Parms);

	if (IsTurn != nullptr)
		*IsTurn = Parms.IsTurn;
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.OnFail_1EB8A4B348D9F7922741CD9438D88086
// (BlueprintCallable, BlueprintEvent)
// Parameters:
// EPathFollowingResult                    MovementResult                                         (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UBP_AIAction_OtomoFollow_C::OnFail_1EB8A4B348D9F7922741CD9438D88086(EPathFollowingResult MovementResult)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "OnFail_1EB8A4B348D9F7922741CD9438D88086");

	Params::BP_AIAction_OtomoFollow_C_OnFail_1EB8A4B348D9F7922741CD9438D88086 Parms{};

	Parms.MovementResult = MovementResult;

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.OnFail_F8B4E13F434B182E285FEEA53BAAED7C
// (BlueprintCallable, BlueprintEvent)
// Parameters:
// EPathFollowingResult                    MovementResult                                         (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UBP_AIAction_OtomoFollow_C::OnFail_F8B4E13F434B182E285FEEA53BAAED7C(EPathFollowingResult MovementResult)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "OnFail_F8B4E13F434B182E285FEEA53BAAED7C");

	Params::BP_AIAction_OtomoFollow_C_OnFail_F8B4E13F434B182E285FEEA53BAAED7C Parms{};

	Parms.MovementResult = MovementResult;

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.OnSuccess_1EB8A4B348D9F7922741CD9438D88086
// (BlueprintCallable, BlueprintEvent)
// Parameters:
// EPathFollowingResult                    MovementResult                                         (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UBP_AIAction_OtomoFollow_C::OnSuccess_1EB8A4B348D9F7922741CD9438D88086(EPathFollowingResult MovementResult)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "OnSuccess_1EB8A4B348D9F7922741CD9438D88086");

	Params::BP_AIAction_OtomoFollow_C_OnSuccess_1EB8A4B348D9F7922741CD9438D88086 Parms{};

	Parms.MovementResult = MovementResult;

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.OnSuccess_F8B4E13F434B182E285FEEA53BAAED7C
// (BlueprintCallable, BlueprintEvent)
// Parameters:
// EPathFollowingResult                    MovementResult                                         (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UBP_AIAction_OtomoFollow_C::OnSuccess_F8B4E13F434B182E285FEEA53BAAED7C(EPathFollowingResult MovementResult)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "OnSuccess_F8B4E13F434B182E285FEEA53BAAED7C");

	Params::BP_AIAction_OtomoFollow_C_OnSuccess_F8B4E13F434B182E285FEEA53BAAED7C Parms{};

	Parms.MovementResult = MovementResult;

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.PlayerCameraCheck
// (Private, HasOutParams, BlueprintCallable, BlueprintEvent, BlueprintPure)
// Parameters:
// bool*                                   InCamera                                               (Parm, OutParm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UBP_AIAction_OtomoFollow_C::PlayerCameraCheck(bool* InCamera)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "PlayerCameraCheck");

	Params::BP_AIAction_OtomoFollow_C_PlayerCameraCheck Parms{};

	UObject::ProcessEvent(Func, &Parms);

	if (InCamera != nullptr)
		*InCamera = Parms.InCamera;
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.PlayerJumpEvent
// (BlueprintCallable, BlueprintEvent)
// Parameters:
// class UPalCharacterMovementComponent*   Component                                              (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, InstancedReference, NoDestructor, HasGetValueTypeHash)

void UBP_AIAction_OtomoFollow_C::PlayerJumpEvent(class UPalCharacterMovementComponent* Component)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "PlayerJumpEvent");

	Params::BP_AIAction_OtomoFollow_C_PlayerJumpEvent Parms{};

	Parms.Component = Component;

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.SetInitialValue
// (Public, BlueprintCallable, BlueprintEvent)

void UBP_AIAction_OtomoFollow_C::SetInitialValue()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "SetInitialValue");

	UObject::ProcessEvent(Func, nullptr);
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.Speed Control
// (Public, BlueprintCallable, BlueprintEvent)
// Parameters:
// double                                  DeltaTime                                              (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UBP_AIAction_OtomoFollow_C::Speed_Control(double DeltaTime)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "Speed Control");

	Params::BP_AIAction_OtomoFollow_C_Speed_Control Parms{};

	Parms.DeltaTime = DeltaTime;

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.targetPosition_2
// (Public, BlueprintCallable, BlueprintEvent)

void UBP_AIAction_OtomoFollow_C::targetPosition_2()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "targetPosition_2");

	UObject::ProcessEvent(Func, nullptr);
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.TryWarpToPlayer
// (Private, BlueprintCallable, BlueprintEvent)

void UBP_AIAction_OtomoFollow_C::TryWarpToPlayer()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "TryWarpToPlayer");

	UObject::ProcessEvent(Func, nullptr);
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.UpdateReturnTooFar
// (Public, BlueprintCallable, BlueprintEvent)

void UBP_AIAction_OtomoFollow_C::UpdateReturnTooFar()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "UpdateReturnTooFar");

	UObject::ProcessEvent(Func, nullptr);
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.GetFollowInterpolatedPosFromController
// (Public, HasOutParams, BlueprintCallable, BlueprintEvent, BlueprintPure, Const)
// Parameters:
// struct FVector*                         FollowInterpolatedPos                                  (Parm, OutParm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UBP_AIAction_OtomoFollow_C::GetFollowInterpolatedPosFromController(struct FVector* FollowInterpolatedPos) const
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "GetFollowInterpolatedPosFromController");

	Params::BP_AIAction_OtomoFollow_C_GetFollowInterpolatedPosFromController Parms{};

	UObject::ProcessEvent(Func, &Parms);

	if (FollowInterpolatedPos != nullptr)
		*FollowInterpolatedPos = std::move(Parms.FollowInterpolatedPos);
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.GetFollowSpeedFromController
// (Public, HasOutParams, BlueprintCallable, BlueprintEvent, BlueprintPure, Const)
// Parameters:
// double*                                 FollowSpeed                                            (Parm, OutParm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UBP_AIAction_OtomoFollow_C::GetFollowSpeedFromController(double* FollowSpeed) const
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "GetFollowSpeedFromController");

	Params::BP_AIAction_OtomoFollow_C_GetFollowSpeedFromController Parms{};

	UObject::ProcessEvent(Func, &Parms);

	if (FollowSpeed != nullptr)
		*FollowSpeed = Parms.FollowSpeed;
}


// Function BP_AIAction_OtomoFollow.BP_AIAction_OtomoFollow_C.TryGetTrainer
// (Public, HasOutParams, BlueprintCallable, BlueprintEvent, BlueprintPure, Const)
// Parameters:
// class APalCharacter**                   Trainer_0                                              (Parm, OutParm, ZeroConstructor, NoDestructor, HasGetValueTypeHash)

void UBP_AIAction_OtomoFollow_C::TryGetTrainer(class APalCharacter** Trainer_0) const
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_AIAction_OtomoFollow_C", "TryGetTrainer");

	Params::BP_AIAction_OtomoFollow_C_TryGetTrainer Parms{};

	UObject::ProcessEvent(Func, &Parms);

	if (Trainer_0 != nullptr)
		*Trainer_0 = Parms.Trainer_0;
}


SDK_NAMESPACE_END
