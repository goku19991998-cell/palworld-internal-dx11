...
namespace DX11_Base {
	...
	bool D3D11Window::Init(IDXGISwapChain* swapChain)
	{
		if (SUCCEEDED(swapChain->GetDevice(__uuidof(ID3D11Device), (void**)&m_Device))) {
			...
			//Chinese language support. Remove below line it if not needed.
			//io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\msjhbd.ttc", 18.0f, NULL, io.Fonts->GetGlyphRangesChineseFull());
